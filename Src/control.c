...
flag createCommand(Tcl_CmdProc *proc, char *name) {
  if (!Tcl_CreateCommand(Interp, name, proc, 
...
flag registerCommand(Tcl_CmdProc *proc, char *name, char *synopsis) {
  if (proc) {
...
flag unsafeToRun(char *command, mask badTasks) {
  Task T;
  for (T = Tasks; T; T = T->prev)
    if (T->type & badTasks)
