...
extern void cleanupGroupType(void *V) {
  ...
  if (!special && !(G->inputType & MAX_POOLING)) G->type |= BIASED;
  ...
  if (!(G->inputType & BASIC_INPUT_TYPES) && !special) {
    ...
    else if (G->outputType & KWTA)
      prependProc(G, GROUP_INPUT, DISTANCE_MISMATCH);
    else
  ...
  if (!(G->outputType & (BASIC_OUTPUT_TYPES | CLAMPING_OUTPUT_TYPES))) {
    ...
    else if (G->inputType & MAX_POOLING)
      prependProc(G, GROUP_OUTPUT, LINEAR);
  ...
  if (!(G->costType & ERROR_MASKS) && (G->type & OUTPUT) && 
    ...
    if (G->outputType & SOFT_MAX)
      ...
      appendProc(G, GROUP_COST, SUM_SQUARED);
    else if (G->outputType & RELU)
      appendProc(G, GROUP_COST, RELU_SUM_SQUARED);
    else
