...
ExampleSet newExampleSet(char *name) {
  ...
  S->regraceTime        = DEF_S_regraceTime;
...
void initEvent(Event V, Example E) {
  ...
  V->regraceTime   = DEF_V_regraceTime;
...
static flag normalEvent(Event V, ExampleSet S) {
  ...
  if (!SAME(V->regraceTime, DEF_V_regraceTime))  return FALSE;
...
static flag readExample(Example E, ParseRec R) {
  ...
  while (!stringMatch(R, ";")) {
	...
	else if (stringMatch(R, "regrace:")) {
	  if (readReal(R, &V->regraceTime)) {
	    parseError(R, "missing value after \"regrace:\" in event header");
	    goto abort;}
	  for (w = v + 1; w < E->numEvents; w++)
	    if (eventActive[w]) E->event[w].regraceTime = V->regraceTime;
	}
...
static flag readExampleSet(char *setName, char *fileName, ExampleSet *Sp, 
  ...
  while (!stringMatch(R, ";")) {
	...
	goto abort;}
      ...
      if (readReal(R, &(S->regraceTime))) {
	parseError(R, "missing value after \"regrace:\" in the set header");
	goto abort;}
    }
    else if (stringMatch(R, "defI:")) {
...
static flag readBinaryExample(Example E, ParseRec R) {
  ...
  for (i = 0; i < sets; i++) {
    ...
    if (readBinReal(channel, &V->regraceTime))
      return parseError(R, "couldn't read event regraceTime");
...
static flag readBinaryExampleSet(char *setName, ParseRec R, ExampleSet *Sp,
  ...
  if (readBinReal(channel, &S->graceTime)) {
    ...
    goto abort;}
  if (readBinReal(channel, &S->regraceTime)) {
    parseError(R, "couldn't read set regraceTime");
    goto abort;}
...
flag writeExampleFile(ExampleSet S, char *fileName, flag append) {
  ...
  if (!append) {
    ...
    if (!SAME(S->regraceTime, DEF_S_regraceTime))
      writeReal(channel, S->regraceTime, "\nregrace:", "");
  ...
  for (E = S->firstExample, halted = FALSE; E && !halted; E = E->next) {
	...
	if (!SAME(V->regraceTime, DEF_V_regraceTime))
	  writeReal(channel, V->regraceTime, " regrace:", "");
...
flag writeBinaryExampleFile(ExampleSet S, char *fileName) {
  ...
  */
  ...
  writeBinReal(channel, S->regraceTime);
  ...
  for (E = S->firstExample, halted = FALSE; E && !halted; E = E->next) {
	...
	writeBinString(channel, s, len);
      ...
      writeBinReal(channel, V->regraceTime);
...
flag moveExamples(ExampleSet from, char *toName, int first, int num, 
  ...
  if (!(to = lookupExampleSet(toName))) {
    ...
    to->regraceTime        = from->regraceTime;
