...
flag addGroup(char *name, int numTypes, unsigned int *typeClass, mask *type, 
  ...
  G->elbowPoint		= 0.5 ;
  G->elbowLowerSlope	= 1.0 ;
  G->elbowUpperSlope	= 2.0 ;
  G->maxTick		= 1.0 ;
  G->IPSPthreshold	= .5  ;
  ...
  // dcp
  G->outputGain		    = NaN ;
  G->noZeroInputNoise	    = 0 ;
  G->applyNoiseOnce	    = 0 ;
  G->integratedOutputTau    = NaN ;
  G->integratedOutputDecay  = NaN ;
  ...
  G->dropoutProb        = DEF_G_dropoutProb;
  ...
  G->k			= DEF_G_k;
  G->mismatchCriterion  = DEF_G_mismatchCriterion;
  G->mismatchPenalty	= DEF_G_mismatchPenalty;
...
flag addNet(char *name, mask type, int timeIntervals, 
  ...
  // dcp
  N->outputGain		    = 1.0 ;
  N->integratedOutputTau    = 1.0 ;
  N->integratedOutputDecay  = 0.0 ;
  N->lastFeedforwardGroupNum = 0 ;
  ...
  N->dropoutProb     = DEF_N_dropoutProb;
