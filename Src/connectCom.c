...
int C_copyConnect(TCL_CMDARGS) {
  ...
  // dcp
  else if (subString(argv[3], "integratedOutput", 7)) 
    offset = OFFSET(U, integratedOutput);
...
int C_noiseType(TCL_CMDARGS) {
  flag mult = FALSE, flat = FALSE, uhalf = FALSE, ghalf = FALSE, rangeSet = FALSE;
  real range = NaN, (*proc)(real value, real range);
  int arg;
  char *usage = "noiseType <group-list> [-range <range> | -multiply | -flat | -uhalf | -ghalf ]";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc < 2) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  ...
  if (flat)
    ...
    proc = addHalfUniformNoise;
  else if (ghalf)
    proc = addHalfGaussianNoise;
  else
