...
struct network {
  ...
  real       gain;
  real	     outputGain;
  real	     integratedOutputTau ;
  real	     integratedOutputDecay ;
  // allows for initial feedforward layers (run only once) in CONTINUOUS networks
  int	     lastFeedforwardGroupNum ;
...
struct group {
  ...
  real       gain;
  real	     outputGain;
  flag       noZeroInputNoise;
  flag       applyNoiseOnce;
  real	     integratedOutputTau ;
  real	     integratedOutputDecay ;
  real	     sumActivation ;		/* hidden */
  ...
  real 	     elbowPoint;
  real 	     elbowLowerSlope;
  real 	     elbowUpperSlope;
  real	     maxTick;
  real	     IPSPthreshold;
  ...
  // for KWTA (arrays are local storage to identify top k)
  int	     k;
  int	     *kIndex;
  real	     *kAct;
  real	     mismatchCriterion;
  real	     mismatchPenalty;
