...
// assumes original values are in "from" group
static int C_splitGroupExternalInput(TCL_CMDARGS) {
  real factor1, factor2 ;
  Group H ;
  char *usage = "splitGroupExternalInput <from-group-list> <to-group-list> <factor1> <factor2>";
  if (argc == 2 && !strcmp(argv[1], "-h"))
    return commandHelp(argv[0]);
  ...
  FOR_EACH_GROUP_IN_LIST(argv[2], {
	  ...
	  FOR_EACH_UNIT_PAIR(G, H, {
	      V->externalInput = factor2 * U->externalInput ;
	      U->externalInput *= factor1 ;
	    });
	});
    });
  return TCL_OK;
...
static int C_addBackgroundExample(TCL_CMDARGS) {
  real threshold, brightness, *value ;
  int  other ;
  ExampleSet S;
  char *usage = "addBackgroundExample <group-list> <exampleSet> <threshold> <brightness>";
  if (argc == 2 && !strcmp(argv[1], "-h"))
    return commandHelp(argv[0]);
  if (argc != 5) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  S          = lookupExampleSet(argv[2]) ;
  threshold  = ator(argv[3]) ;
  brightness = ator(argv[4]) ;
  other      = randInt(S->numExamples) ;
  value      = S->example[other]->event[0].input->val ;

  FOR_EACH_GROUP_IN_LIST(argv[1], {
      FOR_EVERY_UNIT(G, {
	  if (U->externalInput < threshold)
	    U->externalInput = value[u] * brightness ;
	});
    });
  return TCL_OK;
}
...
// Weight Sharing
//
// first create non-shared links/weights before setting up sharing
//
// sharing incoming weights (done by sharing data structures)
//   shareWeights <group>
//
// sharing outgoing weights (done "manually" by accumulating corresponding derivatives)
//   compileOutgoingLinks <group>
//   equateOutgoingWeights <group>
//   setObj postEpochProc { sumOutgoingDerivs <group> }
//
// if multiple groups are sharing (within each group), can list all of them
//   in the same command:
//     shareWeights hidden1 hidden2 hidden3 .....
//

static int C_shareWeights(TCL_CMDARGS) {
  char *usage = "shareWeights <group-list>";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc != 2) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  FOR_EACH_GROUP_IN_LIST(argv[1], {
      FOR_EACH_UNIT(G, {
	  Unit U0 = G->unit ;
	  if (U == U0) continue ;  // skip first unit
	  free(U->incoming) ;
	  free(U->incoming2) ;
	  U->incoming  = U0->incoming ;
	  U->incoming2 = U0->incoming2 ;
	});
    });
  return TCL_OK;
}

static int C_compileOutgoingLinks(TCL_CMDARGS) {
  int i;
  Group G;
  char *_l ;
  flag _code;
  String _name = newString(32);
  char *usage = "compileOutgoingLinks <group-list>";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc != 2) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  ...
  while ((_l = readName(_l, _name, &_code))) {
    ...
    FOR_EACH_UNIT(G, { // U
	// count U's outgoing links
	U->ext->numOutgoing = 0 ;
	FOR_EACH_GROUP_H({ // H
	    FOR_EACH_UNIT_V(H, { // V
		FOR_EACH_LINK_AND_SENDING_UNIT2(V, { // V2-->V (L2)
		    if (U == V2) U->ext->numOutgoing++ ;
		  }) ;
	      }) ;
	  });
	// create array of pointers to outgoing derivaties
	U->ext->outgoingLinks = (Link *)malloc(sizeof(Link) * U->ext->numOutgoing) ;
	// store pointers to links
	i = 0 ;
	FOR_EACH_GROUP_H({
	    FOR_EACH_UNIT_V(H, {
		FOR_EACH_LINK_AND_SENDING_UNIT2(V, {
		    if (U == V2) U->ext->outgoingLinks[i++] = L2 ;
		  }) ;
	      }) ;
	  });
      });
...
static int C_equateOutgoingWeights(TCL_CMDARGS) {
  // must have already called compileOutgoingLinks
  int i ;
  char *usage = "equateOutgoingWeights <group-list>";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc != 2) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  FOR_EACH_GROUP_IN_LIST(argv[1], {
      FOR_EACH_UNIT(G, {  
	  Unit U0 = G->unit ;
	  // copy from first unit's weights
	  if (U == U0) continue ;  
	  for (i = 0 ; i < U0->ext->numOutgoing ; i++)
	    U->ext->outgoingLinks[i]->weight = U0->ext->outgoingLinks[i]->weight ;
	});
    });
  return TCL_OK;
}

static int C_sumOutgoingDerivs(TCL_CMDARGS) {
  // must have already called compileOutgoingLinks
  int i ;
  char *usage = "sumOutgoingDerivs <group-list>";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc != 2) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  FOR_EACH_GROUP_IN_LIST(argv[1], {
      FOR_EACH_UNIT(G, {  
	  Unit U0 = G->unit ;
	  // accumulate in first unit's derivs
	  if (U == U0) continue ;
	  for (i = 0 ; i < U0->ext->numOutgoing ; i++)
	    U0->ext->outgoingLinks[i]->deriv += U->ext->outgoingLinks[i]->deriv ;
	});
      FOR_EACH_UNIT(G, {  
	  Unit U0 = G->unit ;
	  // copy from first unit's derivs
	  if (U == U0) continue ;  
	  for (i = 0 ; i < U0->ext->numOutgoing ; i++)
	    U->ext->outgoingLinks[i]->deriv = U0->ext->outgoingLinks[i]->deriv ;
	});
...
// symmetrizeWeights
//
// sets weights on bidirectional connections to their average
// -w   weight by relative sizes of groups (maintaining sum of original weights)

static void symmetrizeUnitWeights(Unit U, mask linkType, flag weighted) {
  real Lsize, L2size, Lprop, L2prop, wsum ;
  Lsize = (real)U->group->numUnits ;
  // forward: V->U
  FOR_EACH_LINK_AND_SENDING_UNIT(U, {
      if (LINK_TYPE_MATCH((int) linkType, B)) {
	if (weighted) {
	  L2size = (real)(V->group->numUnits) ;
	  Lprop  = Lsize  / (Lsize + L2size) ;
	  L2prop = L2size / (Lsize + L2size) ;
	} else
	  Lprop = L2prop = 0.5 ;
	// backward: V2(=U?)->V  (note V rather than U in call below)
	FOR_EACH_LINK_AND_SENDING_UNIT2(V, {
	    if (U == V2) {
	      wsum = L->weight + L2->weight ;
	      L->weight  = Lprop  * wsum ;
	      L2->weight = L2prop * wsum ;
	    }
	  });
      }
    });
...
int C_symmetrizeWeights(TCL_CMDARGS) {
  mask linkType = ALL_LINKS;
  flag weighted = FALSE ;
  int arg;
  char *groupList = NULL, *unitList = NULL;
  char *usage = "symmetrizeWeights> [-group <group-list> |\n"
    "\t-unit <unit-list> | -type <link-type>] [-weighted]";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  ...
  if (groupList) {
	...
	FOR_EACH_UNIT(G, {
	    symmetrizeUnitWeights(U, linkType, weighted) ;
	  });
      });
  }
  if (unitList) {
...
// zeroSmallWeights
//
// sets weights < magnitude to zero


static void zeroSmallUnitWeights(Unit U, mask linkType, real magnitude) {
  FOR_EACH_LINK_AND_SENDING_UNIT(U, {
      if (LINK_TYPE_MATCH((int) linkType, B)) {
	if (fabs(L->weight) < magnitude) L->weight = 0.0 ;
      }
    });
}

int C_zeroSmallWeights(TCL_CMDARGS) {
  mask linkType = ALL_LINKS;
  int arg;
  float magnitude ;
  char *groupList = NULL, *unitList = NULL;
  char *usage = "zeroSmallWeights> [-group <group-list> |\n"
    "\t-unit <unit-list> | -type <link-type>] -magnitude <real>";
  if (argc == 2 && !strcmp(argv[1], "-h")) 
    return commandHelp(argv[0]);
  if (argc < 1) return usageError(argv[0], usage);
  if (!Net) return warning("%s: no current network", argv[0]);
  ...
  if (groupList) {
    FOR_EACH_GROUP_IN_LIST(groupList, {
	FOR_EACH_UNIT(G, {
	    zeroSmallUnitWeights(U, linkType, magnitude) ;
	  });
      });
  }
  if (unitList) {
...
/* average SSE (over units) must fall below criterion */
static flag aveSSEGroupCriterion(Group G, real criterion) {
  real sum = 0.0 ;
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) sum += SQUARE(U->output - U->target) ;
  });
  if (sum / (float)G->numUnits < criterion)
    return TRUE ;
  return FALSE;
...
static void aveSSEGroupCriterionInit(Group G, GroupProc P) {
  G->groupCriterionReached = aveSSEGroupCriterion;
}

/* average activation (over units) must exceed criterion */
static flag aveActGroupCriterion(Group G, real criterion) {
  real sum = 0.0 ;
  FOR_EACH_UNIT(G, {
      sum += U->output ;
  });
...
static void aveActGroupCriterionInit(Group G, GroupProc P) {
  G->groupCriterionReached = aveActGroupCriterion;
}
...
flag userInit(void) {
  ...
  registerCommand(C_splitGroupExternalInput, "splitGroupExternalInput", "distribute fraction of external inputs to another group");
  registerCommand(C_addBackgroundExample, "addBackgroundExample", "add second example inputs in background at reduced contrast");
  ...
  registerCommand(C_shareWeights, "shareWeights", "share incoming weights among units in a group"); 
  registerCommand(C_compileOutgoingLinks, "compileOutgoingLinks", "create array of pointers to outgoing links"); 
  registerCommand(C_equateOutgoingWeights, "equateOutgoingWeights", "equate weights on outgoing links"); 
  registerCommand(C_sumOutgoingDerivs, "sumOutgoingDerivs", "accumulate derivatives on (shared) outgoing links"); 
  registerCommand(C_topoWeightDecay,   "topoWeightDecay",    "scale weight decay by topographic distance");
  registerCommand(C_symmetrizeWeights, "symmetrizeWeights",  "make bidirectional weights symmetric by averaging");
  registerCommand(C_zeroSmallWeights,  "zeroSmallWeights",   "set weights < magnitude to zero");
  ...
  registerGroupType("AVESSE_CRIT",  AVESSE_CRIT,   GROUP,  aveSSEGroupCriterionInit);
  registerGroupType("AVEACT_CRIT",  AVEACT_CRIT,   GROUP,  aveActGroupCriterionInit);
