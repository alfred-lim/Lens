...
// dcp
// these versions move 0/1 extremes towards middle
real addHalfGaussianNoise(real value, real range) {
  if (value < 0.5) return value + fabs(randGaussian(0.0, range));
  return value - fabs(randGaussian(0.0, range));
}
real addHalfUniformNoise(real value, real range) {
  real r = range * (real)drand48() ;
  return (value < 0.5 ? value + r : value - r) ;
}
