...
void resetOutputs(Group G) {
  real *O = G->output, 
    initInput = chooseValue(G->initInput, Net->initInput),
  ...
  if (!G->inputProcs || G->outputType & HARD_CLAMP) {
    ...
    // dcp: need to reset input for IN_INTEGR
    if (G->inputType & IN_INTEGR) {
      FOR_EACH_UNIT2(G, U->input = initInput) ;
    }
  }
}
...
// dcp: needed for not RESET_ON_EXAMPLE (particularly with IN_INTEGR)
void restoreFinalForwardIntegrators(Group G) {
  GroupProc P;
  int lastTick = G->net->ticksOnExample - 1 ;
  if (G->inputType & IN_INTEGR) {
    //    real val = chooseValue(G->initInput, Net->initInput);
    for (P = G->inputProcs; P; P = P->next)
      if (P->type == IN_INTEGR)
	FOR_EACH_UNIT2(G, P->unitData[u] = GET_HISTORY(U, inputHistory, lastTick)) ;
  }
  if (G->outputType & (OUT_INTEGR | INTERACT_INTEGR)) {
	...
	FOR_EACH_UNIT2(G, P->unitData[u] = GET_HISTORY(U, outputHistory, lastTick)) ;
  }
}
...
void resetGroup (Group G) {
  ...
  if (G->type & IPSP_DYNAMIC) 
    FOR_EACH_UNIT(G, U->activeTicks = 0 ) ;
  if (G->type & (INTEGR_MULT_DECAY|INTEGR_MULT_DECAY2))
    FOR_EACH_UNIT(G, U->integratedOutput = 0.0 ) ;
}
...
static void dotProductInputBack(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, {
	...
	// V_DRV is outputDeriv (cache) of sending unit(s)
	// L_DRV is deriv of corresponding incoming link(s)
...
// NOTE: mismatchCriterion is on squared difference; mismatchPenalty is multiplicative
static void distanceMismatchInput(Group G, GroupProc P) {
  real crit = G->mismatchCriterion, factor = G->mismatchPenalty ;
  FOR_EACH_UNIT(G, {
    real input = 0.0;
    FOR_EACH_LINK_FORW(U, {
	real dist = SQUARE(L_WGT - V_OUT) ;
	if (dist < crit) input += dist ;
	else             input += dist*factor ;
      });
    U->input = input;
  });
}

// not considering mismatchPenalty (should?)
static void distanceMismatchInputBack(Group G, GroupProc P) {
  real delta;
  FOR_EACH_UNIT(G, {
    real inputDeriv = U->inputDeriv * 2.0;
    if (inputDeriv != 0.0) {
...
static void distanceMismatchInputInit(Group G, GroupProc P) {
  P->forwardProc  = distanceMismatchInput;
  P->backwardProc = distanceMismatchInputBack;
}
...
// dcp: "center" inputs (subtract mean, divide by sd)
static void centerInput(Group G, GroupProc P) {
  int tick = HISTORY_INDEX(Net->currentTick);
  real *centeredInput = P->unitHistoryData[tick];
  real mean = 0.0, sd = 0.0, scale = 1.0 ;
  FOR_EACH_UNIT(G, mean += U->input);
  mean /= (real)G->numUnits ;
  FOR_EACH_UNIT(G, sd += SQUARE(U->input - mean));
  sd = sqrt(sd / (real)G->numUnits) ;
  if (sd > 0.0) scale = 1.0 / sd ;
  FOR_EACH_UNIT2(G, {U->input = scale * (U->input - mean); centeredInput[u] = U->input; });
  P->groupHistoryData[tick] = scale;
}

// static void centerInputBack(Group G, GroupProc P) {
//   int tick = HISTORY_INDEX(Net->currentTick);
//   real shift = 0.0, scale = P->groupHistoryData[tick],
//     *centeredInput = P->unitHistoryData[tick];
//   FOR_EACH_UNIT2(G, shift += U->inputDeriv * centeredInput[u]);
//   FOR_EACH_UNIT(G, U->inputDeriv = scale * (U->inputDeriv - shift););
// }
...
static void centerInputInit(Group G, GroupProc P) {
  P->forwardProc  = centerInput;
  P->backwardProc = centerInputBack;
  P->groupHistoryData = realArray(Net->historyLength, 
				  "IN_CENTER:groupHistoryData");
  P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				  "IN_CENTER:unitHistoryData");
}
...
static void noisyInput(Group G, GroupProc P) {
  if (G->noiseProc) {
    real range = chooseValue(G->noiseRange, Net->noiseRange);
    FOR_EACH_UNIT(G, {
	// dcp: if set to 1, don't add noise to units with specified externalInput = 0.0
	if (!G->noZeroInputNoise || U->externalInput > 0.0)
	  U->input = G->noiseProc(U->input, range) ;
    });
  }
}
...
static void maxPoolingInput(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, {
    real max = -1.0;
    FOR_EACH_LINK_FORW(U, { if (V_OUT > max) max = V_OUT ;});
    U->input = max;
  });
}

static void maxPoolingInputBack(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, {
    real inputDeriv = U->inputDeriv ;
    if (inputDeriv != 0.0) {
      real *deriv ;
      real max = -1.0 ;
      // inefficient to recompute max; should store index in forward pass
      FOR_EACH_LINK_BACK(U,
	if (V_OUT > max) {
	  max = V_OUT ;
	  deriv = &V_DRV ;
...
static void maxPoolingInputInit(Group G, GroupProc P) {
  P->forwardProc  = maxPoolingInput;
  P->backwardProc = maxPoolingInputBack;
}
...
// dcp
static void rectifiedLinearOutput(Group G, GroupProc P) {
  real drop = chooseValue(G->dropoutProb, Net->dropoutProb);
  if (drop <= 0.0) {
    FOR_EACH_UNIT(G, U->output = (U->input <= 0.0 ? 0.0 : U->input));
  } else {
    FOR_EACH_UNIT(G, U->output = ((U->input <= 0.0 || randProb()<drop) ? 0.0 : U->input));
  }
}

static void rectifiedLinearOutputBack(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, U->inputDeriv = (U->output > 0.0 ? U->outputDeriv : 0.0)) ;
}

static void rectifiedLinearOutputInit(Group G, GroupProc P) {
  P->forwardProc  = rectifiedLinearOutput;
  P->backwardProc = rectifiedLinearOutputBack;
}
...
static void elbowOutput(Group G, GroupProc P) {
	...
	real lower = G->elbowLowerSlope, upper = G->elbowUpperSlope ;
	FOR_EACH_UNIT2(G,
	if (originalOutput[u] < G->elbowPoint){
		U->output = lower * U->input;
		}
	else {
		U->output = upper * U->input;
	}
	);
}
...
static void elbowOutputBack(Group G, GroupProc P){
	...
	FOR_EACH_UNIT2(G,
		if (originalOutput[u] < G->elbowPoint)
		{ U->inputDeriv = lower * U->outputDeriv;}
		else
		{ U->inputDeriv = upper * U->outputDeriv;} 
	);
}
...
// dcp
static void rectifiedTanhOutput(Group G, GroupProc P) {
  real x, drop = chooseValue(G->dropoutProb, Net->dropoutProb);
  if (G->type & ADAPTIVE_GAIN) {
    if (drop > 0.0) {
      FOR_EACH_UNIT(G, {
	  x = U->gain * U->input ;
	  U->output = ((U->input <= 0.0 || randProb()<drop) ? 0.0 : CALC_RTANH(x)) ;
	}) ;
    } else {
      FOR_EACH_UNIT(G, {
	  x = U->gain * U->input ;
	  U->output = CALC_RTANH(x) ;
	});
    }
  } else {
    real gain = chooseValue(G->gain, Net->gain);
...
static void rectifiedTanhOutputBack(Group G, GroupProc P) {
  if (G->type & ADAPTIVE_GAIN) {
    real x;
    FOR_EACH_UNIT(G, {
	x = (U->output > 0.0 ? U->outputDeriv * RTANH_DERIV(U->input) : 0.0) ;
	U->inputDeriv = x * U->gain;
	U->gainDeriv += x * U->input;
      });
  } else {
    real gain = chooseValue(G->gain, Net->gain);
    FOR_EACH_UNIT(G, {
	U->inputDeriv = (U->output > 0.0 ? U->outputDeriv * gain * RTANH_DERIV(U->input) : 0.0) ;
      });
  }
}

static void rectifiedTanhOutputInit(Group G, GroupProc P) {
  P->forwardProc  = rectifiedTanhOutput;
  P->backwardProc = rectifiedTanhOutputBack;
}
...
// uses DISTANCE input function (lower is better)
static void kWTAOutput(Group G, GroupProc P) {
  real kthA, *kA = G->kAct ;
  int i, kthI = 0, k = G->k, *kI = G->kIndex ;
  if (k > G->numUnits) {
    printf("kWTAOutput: G->k (%d) > G->numUnits (%d)\n", G->k, G->numUnits) ;
    exit(0);
  }
  // kthA/kthI are the activation/index (in kA) of the kth (worst) unit
  FOR_EACH_UNIT2(G, {
      real sAct = U->input ;
      // store first k activations/indices, identifying worst (kth) unit
      if (u < k) {
	if (u == 0) kthA = sAct ;
	kA[u] = sAct ;
	kI[u] = u ;
	...
	// if current unit's activation is better than current kth, replace...
	if (sAct < kthA) {	// better
	  kA[kthI] = kthA = sAct ;
	  kI[kthI] = u ;
	  // ...and determine new worst (kth) unit
	  for (i = 0 ; i < k ; i++)
	    if (kA[i] > kthA) {	// worse
	      kthA = kA[i] ;
	      kthI = i ;
  ...
  for (i = 0 ; i < k ; i++) G->unit[kI[i]].output = 1.0 ; 
}

static void kWTAOutputBack(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, {
      U->inputDeriv = U->output ;
    }) ;
}

static void kWTAOutputInit(Group G, GroupProc P) {
  P->forwardProc  = kWTAOutput;
  P->backwardProc = kWTAOutputBack;
  P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits, 
				  "kWTAOutputInit:unitHistoryData");
  G->kIndex = (int *)safeMalloc(G->numUnits * sizeof(int), "kWTAOutputInit:G->kIndex") ;
  G->kAct   = (real *)safeMalloc(G->numUnits * sizeof(real), "kWTAOutputInit:G->kAct") ;
}
...
static void noisyOutput(Group G, GroupProc P) {
  ...
  int noiseTick = (G->net->type&CONTINUOUS ? 1 : 0) ;
  if (G->applyNoiseOnce && Net->currentTick > noiseTick) {
    // for INPUT groups, copy (corrupted inputs) forward
    // otherwise, just leave (recomputed) output alone
    if (G->type&INPUT) restoreOutputs(G, Net->currentTick-1) ;
  } else {
    FOR_EACH_UNIT2(G, {
	cleanOutput[u] = U->output;
	if (G->noiseProc &&
	    // dcp: if set to 1, don't add noise to units with specified inputs = 0.0
	    (!G->noZeroInputNoise || cleanOutput[u] > 0.0))
	  U->output = G->noiseProc(U->output, range);
      });
  }
}
...
/* There is no noisyOutputDeriv forward */
...
static void winnerTakeAllOutput(Group G, GroupProc P) {
  ...
  FOR_EACH_UNIT(G, {
      // dcp: just set winner to 1.0 and losers to 0.0
      //    if (U != winner) U->output = G->minOutput;
      U->output = (U == winner ? 1.0 : 0.0);
  });
}
...
// dcp
// integratedOutputDecay is lower bound on decay scale factor (1-integratedOutput)
static void integratedMultDecay(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real tau   = chooseValue(G->integratedOutputTau, Net->integratedOutputTau) ;
	real bound = chooseValue(G->integratedOutputDecay, Net->integratedOutputDecay) ;
	if (G->type&RESET_ON_EXAMPLE && Net->currentTick == 1)
	  FOR_EACH_UNIT2(G, U->integratedOutput = 0.0;) ;
	FOR_EACH_UNIT2(G, {
	  originalOutput[u] = U->output;
	  U->integratedOutput += tau * (U->output - U->integratedOutput);
	  U->output *= 1.0 - U->integratedOutput * (1.0 - bound) ;
	  });
}

// make bound scale between specified bound and 1 as function of intergrated output
static void integratedMultDecay2(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real tau   = chooseValue(G->integratedOutputTau, Net->integratedOutputTau) ;
	real bound = chooseValue(G->integratedOutputDecay, Net->integratedOutputDecay) ;
	if (G->type&RESET_ON_EXAMPLE && Net->currentTick == 1)
	...
	FOR_EACH_UNIT2(G, {
	  originalOutput[u] = U->output;
	  U->integratedOutput += tau * (U->output - U->integratedOutput);
	  bound = 1.0 - (1.0 - bound) * U->integratedOutput ;
	  U->output *= 1.0 - U->integratedOutput * (1.0 - bound) ;
	  });
}
...
static void IPSPDynamic(Group G, GroupProc P){
	...
	FOR_EACH_UNIT2(G, {
		...
		originalOutput[u] = U->output;
		if (U->activeTicks > 0) U->output = U->output * times;
		//print(1, "%f\n", U->output);
		...
		if (IPSPOut > threshold) {
		  ...
		  U->activeTicks-- ;
		  if (U->activeTicks < 0) U->activeTicks = 0 ;
		}
	});
}
...
static void IPSPDynamicAdd(Group G, GroupProc P){
        ...
        FOR_EACH_UNIT2(G, {
                ...
                originalOutput[u] = U->output;
                if (U->activeTicks > 0) U->output = U->output + plus;
                //print(1, "%f\n", U->output);
                real IPSPOut = U->output;
...
// dcp
static void integratedMultDecayBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}
...
// dcp
static void integratedMultDecayInit(Group G, GroupProc P) {
	P->forwardProc = integratedMultDecay;
	P->backwardProc = integratedMultDecayBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"INTEGR_MULT_DECAY:unitHistoryData");
}
static void integratedMultDecay2Init(Group G, GroupProc P) {
	P->forwardProc = integratedMultDecay2;
	P->backwardProc = integratedMultDecayBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"INTEGR_MULT_DECAY2:unitHistoryData");
}
...
static void squaredError(Group G, GroupProc P) {
  ...
  Net->currentExample->error += error;
  G->error += error;
  Net->exampleError += error;
  Net->error += error;
}
...
// dcp rectified linear units (ReLUs); don't penalize if output > target=1
static void ReLUsquaredError(Group G, GroupProc P) {
  real output, target, error = 0.0 ;
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) {
	output = U->output;
	target = U->adjustedTarget = U->target;
	if (target != 1.0 || output < 1.0)
	  error += SQUARE(output - target);
      }
    });
  error *= G->errorScale / Net->ticksPerInterval * 
...
static void ReLUsquaredErrorBack(Group G, GroupProc P) {
  real scale = ((Net->pseudoExampleFreq) ? 
		Net->currentExample->frequency : 1.0) * G->errorScale * 2.0;
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) {
	if (U->target != 1.0 || U->output < 1.0)
	  U->outputDeriv += scale * (U->output - U->target);
      }
    });
}

static void ReLUsquaredErrorInit(Group G, GroupProc P) {
  P->forwardProc  = ReLUsquaredError;
  P->backwardProc = ReLUsquaredErrorBack;
}

// dcp: separate target=0 (T0) and target=1 (T1) units
//      ASSUMES targets are 0 or 1
//      targetRadius is required spread between T0 and T1 means
static void spreadError(Group G, GroupProc P) {
  real numT0 = 0.0, numT1 = 0.0, meanT0 = 0.0, meanT1 = 0.0, error = 0.0, targT0, targT1 ;
  real targMin, targMax, gap = chooseValue(G->targetRadius, Net->targetRadius) ;
  // find T0 and T1 means
  FOR_EACH_UNIT(G, {
      if      (U->target == 0.0) { meanT0 += U->output ; numT0++ ; }
      else if (U->target == 1.0) { meanT1 += U->output ; numT1++ ; }
    });
  meanT0 /= numT0 ;
  meanT1 /= numT1 ;
  targT0 = MAX(0.0,meanT1-gap) ;
  targT1 = MIN(1.0,meanT0+gap) ; 
  targMin = MIN(targT0,targT1) ;
  targMax = MAX(targT0,targT1) ;
  //  printf("T0=%f/%f T1=%f/%f\n", meanT0, targT0, meanT1, targT1);

  // CE error if unit is on wrong side of either boundary
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) {
	U->adjustedTarget = 
	  (U->target == 0.0 ? MIN(U->output,targMin) :
	   (U->target == 1.0 ? MAX(U->output,targMax) :
	    U->target)) ;
	error += CROSS_ENTROPY_ERROR(U->output,U->adjustedTarget) ;
      }
    });
  error *= G->errorScale / Net->ticksPerInterval * 
    ((Net->pseudoExampleFreq) ? Net->currentExample->frequency : 1.0);
...
// dcp: targetRadius is required spread between most active T0 and least active T1
static void spreadErrorExtremes(Group G, GroupProc P) {
  real minT1 = 1.0, maxT0 = 0.0, error = 0.0, targT1, targT0 ;
  real gap = chooseValue(G->targetRadius, Net->targetRadius) ;
  // find maximum T0 and minimum T1 values
  FOR_EACH_UNIT(G, {
      if      (U->target == 0.0 && U->output > maxT0) maxT0 = U->output ;
      else if (U->target == 1.0 && U->output < minT1) minT1 = U->output ;
    });
  //  printf("maxT0=%f minT1=%f\n", maxT0, minT1);
  targT0 = MAX(0.0,minT1-gap) ;
  targT1 = MIN(1.0,maxT0+gap) ; 

  // CE error if unit is on wrong side of relevant boundary
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) {
	U->adjustedTarget = 
	  (U->target == 0.0 ? MIN(U->output,targT0) :
	   (U->target == 1.0 ? MAX(U->output,targT1) :
	    U->target)) ;
	error += CROSS_ENTROPY_ERROR(U->output,U->adjustedTarget) ;
      }
    });
...
static void spreadErrorBack(Group G, GroupProc P) {
  real scale = ((Net->pseudoExampleFreq) ? 
		Net->currentExample->frequency : 1.0) * G->errorScale * 2.0;
  FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) 
	U->outputDeriv += scale * CROSS_ENTROPY_DERIV(U->output,U->adjustedTarget);
    });
}

static void spreadErrorInit(Group G, GroupProc P) {
  P->forwardProc  = spreadError;
  P->backwardProc = spreadErrorBack;
}
...
//
// dcp: reverse of LINEAR_COST: outputCostPeak is actually zero error point
//
static void invLinearCost(Group G, GroupProc P) {
  real cost = 0.0, min = G->minOutput, max = G->maxOutput;
  real p = chooseValue(G->outputCostPeak, Net->outputCostPeak) ;
  if (isNaN(min) || isNaN(max)) {
    FOR_EACH_UNIT(G, cost += ABS(U->output-p));
  } else {
    real invP = 1.0 / (p - min), inv1mP = 1.0 / (max - p);
    FOR_EACH_UNIT(G, cost += (U->output <= p) ? 1.0 - (U->output - min) * invP : 
		  1.0 - (max - U->output) * inv1mP);
  }
  cost *= G->outputCostScale / Net->ticksPerInterval;
  G->outputCost += cost;
  Net->outputCost += cost;
...
static void invLinearCostBack(Group G, GroupProc P) {
  real strength = Net->outputCostStrength * G->outputCostScale /
    Net->ticksPerInterval, min = G->minOutput, max = G->maxOutput;
  real p = chooseValue(G->outputCostPeak, Net->outputCostPeak);
  if (isNaN(min) || isNaN(max)) {
    FOR_EACH_UNIT(G, U->outputDeriv += (U->output > p) ? strength : 
		  (U->output < p) ? -strength : 0.0);
  } else {
      real leftDeriv = strength / (p - min), rightDeriv = strength / (p - max);
    FOR_EACH_UNIT(G, {
      U->outputDeriv += (U->output < p) ? -leftDeriv : 
	(U->output > p) ? -rightDeriv : 0.0;
    });
...
//
// dcp: linear deviation of average unit activation from target value (outputCostPeak)
//
static void aveActCost(Group G, GroupProc P) {
  real cost = 0.0, sum = 0.0 ;
  real target = chooseValue(G->outputCostPeak, Net->outputCostPeak) ;
  FOR_EACH_UNIT(G, sum += U->output) ;
  G->sumActivation = sum ; // used in groupActCostBack
  cost = ABS(sum - target*(real)G->numUnits) * 
         G->outputCostScale/(real)Net->ticksPerInterval;
  G->outputCost += cost;
  Net->outputCost += cost;
}

static void aveActCostBack(Group G, GroupProc P) {
  real strength = Net->outputCostStrength * G->outputCostScale / Net->ticksPerInterval ;
  real target   = chooseValue(G->outputCostPeak, Net->outputCostPeak);
  real ave      = G->sumActivation / (real)G->numUnits ;
  FOR_EACH_UNIT(G, U->outputDeriv += 
		(ave > target) ? strength : (ave < target) ? -strength : 0.0);
}

//
// dcp: penalize activation change
//
static void actChangeCost(Group G, GroupProc P) {
  real strength = Net->outputCostStrength * G->outputCostScale / Net->ticksPerInterval ;
  real cost     = 0.0 ;
  FOR_EACH_UNIT(G, {
      real output = U->outputHistory[Net->currentTick] ;
      real target = U->outputHistory[Net->currentTick-1] ;
      cost += SQUARE(target - output) * strength ;
    }) ;
  G->outputCost += cost;
  Net->outputCost += cost;
}

static void actChangeCostBack(Group G, GroupProc P) {
  real strength = Net->outputCostStrength * G->outputCostScale / Net->ticksPerInterval ;
  FOR_EACH_UNIT(G, {
      real output = U->outputHistory[Net->currentTick] ;
      real target = U->outputHistory[Net->currentTick-1] ;
      U->outputDeriv += (target - output) * strength ;
    }) ;
}


static void invLinearCostInit(Group G, GroupProc P) {
  P->forwardProc  = invLinearCost;
  P->backwardProc = invLinearCostBack;
}

static void aveActCostInit(Group G, GroupProc P) {
  P->forwardProc  = aveActCost;
  P->backwardProc = aveActCostBack;
}

static void actChangeCostInit(Group G, GroupProc P) {
  P->forwardProc  = actChangeCost;
  P->backwardProc = actChangeCostBack;
}
...
void printGroup(Group G, char* tag) {
  printf("%9s[%s]:", G->name, tag) ;
  FOR_EACH_UNIT(G, {
      printf(" %5.3f/%5.3f/%5.3f", U->input, U->output, U->outputDeriv);
      if (!isNaN(U->target)) printf("/%1.0f", U->target);
    }) ;
  printf("\n");
}
...
flag continuousNetTickForward(Event V) {
  ...
  int first = Net->lastFeedforwardGroupNum, last = Net->numGroups - 1 ;
  // store outputs of feedforward layers (for display purposes)
  FOR_EACH_GROUP_IN_RANGE(0,last-1,storeOutputs(G, Net->currentTick));
  // compute outputs of recurrent layers
  FOR_EACH_GROUP_IN_RANGE(first,last,computeInput(G, TRUE));
  FOR_EACH_GROUP_IN_RANGE(first,last,computeOutput(G, TRUE));
  FOR_EACH_GROUP_IN_RANGE(first,last,computeCost(G, TRUE));
  return TCL_OK;
}
...
flag continuousNetTrainTickForward(Event V) {
  ...
  FOR_EACH_GROUP_IN_RANGE(first,last,computeCostBack(G, TRUE));

//   printf("%d:\n", Net->currentTick) ;
//   FOR_EACH_GROUP_IN_RANGE(1,3,printGroup(G,"f"));
//   printf("\n");
...
flag continuousNetExampleBack(Example E) {
  /* Clear the output deriv caches. */
  /* If integrating, reset the deriv accumulators. */
  int last = Net->numGroups - 1, first = Net->lastFeedforwardGroupNum ;
  ...
  for (Net->currentTick = Net->ticksOnExample - 1; 
    ...
    /* When you get here, the outputs are the outputs from Net->currentTick and
       ...
       tick. */
    /* Set outputDerivs to the stored instant error derivatives. */
    FOR_EACH_GROUP_IN_RANGE(first,last,restoreOutputDerivs(G, Net->currentTick));

    /* Compute the instant inputDerivs, first injecting the cache. */
    FOR_EACH_GROUP_IN_RANGE(first,last,computeOutputBack(G));

//     printf("%d:\n", Net->currentTick);
//     FOR_EACH_GROUP_IN_RANGE(1,last,printGroup(G,"b"));
//     printf("\n");

    /* Restore the outputs from the previous tick. */
    if (Net->currentTick > 0) {
      FOR_EACH_GROUP_IN_RANGE(first,last,{
	  restoreOutputs(G, Net->currentTick - 1);
	  restoreInputs(G, Net->currentTick - 1);
	});
    }
    /* Backpropagate across the incoming links. */
    FOR_EACH_GROUP_IN_RANGE(first,last,computeInputBack(G));
  }

  // back-propagate over any initial feedforward layers
  FOR_EACH_GROUP_IN_RANGE_BACK(Net->lastFeedforwardGroupNum,0,BACKPROP(G));

  /* Restore the final outputs for continuity with the next example */
  // dcp: also need to restore inputs when IN_INTEGR and not RESET_ON_EXAMPLE
  FOR_EACH_GROUP_IN_RANGE(first,last,{
      restoreOutputs(G, Net->ticksOnExample - 1);
      if (!(G->type & RESET_ON_EXAMPLE)) {
	if (G->inputType & IN_INTEGR)
	  restoreInputs(G, Net->ticksOnExample - 1);
	restoreFinalForwardIntegrators(G) ;
      }
    });
...
flag standardNetRunExample(Example E, flag (*tickProc)(Event), flag *correct) {
  ...
  real timeOnEvent, minTime, maxTime, graceTime, regraceTime;
  Event V;
  ...
  regraceTime = chooseValue(V->regraceTime, E->set->regraceTime);
  Net->inGracePeriod = (graceTime > 0.0);
  
  /* Reset the outputs and integrators if requested. */
  FOR_EACH_GROUP(if (G->type & RESET_ON_EXAMPLE) resetGroup(G)) ;
  Net->resetHistory[HISTORY_INDEX(0)] = TRUE;
  *correct = TRUE;
  RUN_PROC(preEventProc);

  // dcp: moved this to after preEventProc so it can mess with loaded example
   if (Net->type & CONTINUOUS) {

     // dcp: forward pass for any initial feedforward groups (including bias group)
     Net->currentTick = 0 ;
     FOR_EACH_GROUP_IN_RANGE(0, Net->lastFeedforwardGroupNum, {
	 computeInput(G, TRUE) ;
	 computeOutput(G, TRUE) ;
	 computeCost(G, TRUE) ;
	 if (training) computeCostBack(G, TRUE) ;
       }) ;

     storeOutputsAndTargets(0);
  
     // dcp: also preserve inputs (at least for display; needed for IN_INTEGR)
     FOR_EACH_GROUP(storeInputs(G,0)) ;
  
     Net->exampleHistoryStart = 0;
     Net->eventHistory[0] = -1;	// tick=0 is initial activations
  ...
  for (; tick < Net->maxTicks && event < E->numEvents; tick++) {
    ...
    if (! (Net->type & CONTINUOUS && tick == 0)) { /* don't run tick=0 for CONT net */
    ...
    // dcp: grace period at *end* of event
    if (!isNaN(regraceTime) && timeOnEvent >= regraceTime)
      Net->inGracePeriod = TRUE;

    if (eventDone) {
      if (critReached == 2) critReached = groupCriteriaReached(training);
      // dcp: in case it's needed by  postEventProc
      Net->currentExample->currentEventNum = event;
      if (critReached) {
	Net->currentExample->event[event].critReached = TRUE;
      } else {
	Net->currentExample->event[event].critReached = FALSE;
	*correct = FALSE;
      }
...
flag standardNetTrainBatch(flag *allCorrect) {
  ...
  *allCorrect = TRUE;
  ...
  /* at beginning of epoch, reset groups that won't be reset between examples */
  FOR_EACH_GROUP(if (!(G->type & RESET_ON_EXAMPLE)) resetGroup(G)) ;

  for (i = 0, Net->numExamplesCorrect = 0; i < numExamples; i++) {
    if (loadNextExample(S)) return TCL_ERROR;
    E = S->currentExample;
    E->error = Net->exampleError = 0.0 ;

    RUN_PROC(preExampleProc);
    if (Net->netTrainExample(E, Net->netTrainTick, &criterionReached))
      return TCL_ERROR;

    // dcp: in case it's needed in preExampleBackProc or postExampleProc
    Net->currentExample->critReached = criterionReached;
...
flag standardNetTestBatch(int numExamples, flag resetError, 
  ...
  for (i = 0, totalTicks = 0; numExamples == 0 || i < numExamples; i++) {
    ...
    // dcp
    if (printResults) {
      print(1,"%s %f %d\n", E->name, E->error, criterionReached);
    }
...
void registerNetAndGroupTypes(void) {
  ...
  registerGroupType("IN_COPY", IN_COPY, GROUP_INPUT,
		    copyInputsInit);
  registerGroupType("MAX_POOLING", MAX_POOLING, GROUP_INPUT,
		    maxPoolingInputInit);
  registerGroupType("DISTANCE_MISMATCH", DISTANCE_MISMATCH, GROUP_INPUT,
		    distanceMismatchInputInit);
  registerGroupType("IN_CENTER", IN_CENTER, GROUP_INPUT,
		    centerInputInit);
  ...
  // dcp
  registerGroupType("INTEGR_MULT_DECAY", INTEGR_MULT_DECAY, GROUP_OUTPUT, 
			integratedMultDecayInit);
  registerGroupType("INTEGR_MULT_DECAY2", INTEGR_MULT_DECAY2, GROUP_OUTPUT,
			integratedMultDecay2Init);
  registerGroupType("RTANH", RTANH, GROUP_OUTPUT,
			rectifiedTanhOutputInit);
  registerGroupType("RELU", RELU, GROUP_OUTPUT,
		        rectifiedLinearOutputInit);
  registerGroupType("KWTA", KWTA, GROUP_OUTPUT,
		        kWTAOutputInit);
  ...
  registerGroupType("COSINE", COSINE, GROUP_COST,
		    cosineErrorInit);
  registerGroupType("RELU_SUM_SQUARED", RELU_SUM_SQUARED, GROUP_COST,
		    ReLUsquaredErrorInit);
  registerGroupType("SPREAD_ERROR", SPREAD_ERROR, GROUP_COST,
		    spreadErrorInit);
  ...
  registerGroupType("LINEAR_COST", LINEAR_COST, GROUP_COST,
		    linearCostInit);
  registerGroupType("INVLINEAR_COST", INVLINEAR_COST, GROUP_COST,
		    invLinearCostInit);
  registerGroupType("AVEACT_COST", AVEACT_COST, GROUP_COST,
		    aveActCostInit);
  registerGroupType("ACTCHANGE_COST", ACTCHANGE_COST, GROUP_COST,
		    actChangeCostInit);
