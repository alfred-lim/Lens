  ...
  if (!strcmp(list, "*")) {\
    ...
    while ((l = readName(l, _name, &code))) {\
      ...
      if (!(S = lookupExampleSet(_name->s)))\
        return warning("%s: example set \"%s\" doesn't exist", argv[0], _name->s);\
      proc;\
    }\
    freeString(_name);\
  }}
...
extern flag loadExamples(char *setName, char *fileName, 
			 int mode, int numExamples);
extern flag writeExampleFile(ExampleSet S, char *fileName, flag append);
extern flag writeBinaryExampleFile(ExampleSet S, char *fileName);
extern flag openNetOutputFile(char *fileName, flag binary, flag append);
