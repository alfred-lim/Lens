...
// dcp: really want a permuted client order for asynchronous training
...
static void synchronousContinue(Client C, flag groupCritReached) {
  ...
  if (Clients[IS_TRAINING] == 0) {
	...
	FOR_EACH_CLIENT({
	    if (C->state == IS_IDLE) continue;
	    lastClient = C;
	    C->batchSize = roundr(desiredBatch * C->proportion * totalExamplePerTime);
