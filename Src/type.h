...
#define AVESSE_CRIT   	   ((mask) 1 << 18)
#define AVEACT_CRIT    	   ((mask) 1 << 19)
#define CRITERION_MASKS    (STANDARD_CRIT | MAX_CRIT | GAP_CRIT | FIXED_CRIT | MAXCHANGE_CRIT | AVECHANGE_CRIT | AVESSE_CRIT | AVEACT_CRIT)
...
#define MAX_POOLING	   ((mask) 1 << 12)
#define DISTANCE_MISMATCH  ((mask) 1 << 13)
#define IN_CENTER	   ((mask) 1 << 14)

#define BASIC_INPUT_TYPES  (DOT_PRODUCT | DISTANCE | DISTANCE_MISMATCH | PRODUCT | IN_BOLTZ | \
			    IN_COPY | DOT_PRODUCT_ONCE | MAX_POOLING)
...
//sl
#define INV_SQUARE_DECAY   ((mask) 1 << 21)
#define ELBOW		   ((mask) 1 << 22)
#define IPSP_DYNAMIC	   ((mask) 1 << 23)

// dcp
#define OUT_DECAY          ((mask) 1 << 24)
#define INTEGR_MULT_DECAY  ((mask) 1 << 25)
#define INTEGR_MULT_DECAY2 ((mask) 1 << 26)
#define RTANH	  	   ((mask) 1 << 27)
#define RELU		   ((mask) 1 << 28)
#define RECLU		   ((mask) 1 << 29)
#define KWTA		   ((mask) 1 << 30)

// sl (OLD)
// #define OUT_DECAY_TIME     ((mask) 1 << 23)
// #define OUT_DECAY_EXP	   ((mask) 1 << 24)
// #define URGENCY	  	   ((mask) 1 << 25)
...
#define BASIC_OUTPUT_TYPES (LINEAR | LOGISTIC | TERNARY | TANH | GAUSSIAN | \
                            ...
                            OUT_COPY | INTERACT_INTEGR | RTANH | ELBOW | RELU | RECLU | KWTA )
...
#define RELU_SUM_SQUARED   ((mask) 1 << 4)
#define SPREAD_ERROR       ((mask) 1 << 5)

#define ERROR_MASKS        (SUM_SQUARED | CROSS_ENTROPY | DIVERGENCE | COSINE | RELU_SUM_SQUARED | SPREAD_ERROR )
...
// dcp
#define INVLINEAR_COST     ((mask) 1 << 22)
#define AVEACT_COST	   ((mask) 1 << 23)
#define ACTCHANGE_COST     ((mask) 1 << 24)
#define UNIT_COST_MASKS     (LINEAR_COST | QUADRATIC_COST | CONV_QUAD_COST | \
			     LOGISTIC_COST | COSINE_COST | DELTA_COST | \
			     INVLINEAR_COST | AVEACT_COST | ACTCHANGE_COST)
