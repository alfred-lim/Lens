...
# auto_dirs was added by Doug Rohde
# It is a list of directories in which auto_path has been run
...
# Added by Doug:
    if {$auto_oldpath == $auto_path && [info exists auto_dirs([pwd])]} \
	{return 0}
...
# Added by Doug:
	if {$dir == "."} {set dir [pwd]}
	if [info exists auto_dirs($dir)] continue
	set auto_dirs($dir) 1
	...
	if {$issafe} {
		      ...
		      [catch {set f [open [file join $dir .tclIndex]]}]} {
	    continue
	} else {
	    set error [catch {
		set id [gets $f]
		if {[string equal $id \
		    ...
		    error "[file join $dir tclIndex] or [file join $dir .tclIndex] isn't a proper Tcl index file"
		}
	    } msg]
	    if {[string compare $f ""]} {
		close $f
